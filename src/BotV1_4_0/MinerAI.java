...
//find the best lead
        ...
        if (!moveRandomly&&rc.senseLead(k[indexBest])>1) {
    ...
    static boolean tryToMove(RobotController rc, Direction d) throws GameActionException {
        if (rc.canMove(d) && rc.senseRubble(rc.getLocation().add(d)) < 50) {
            rc.move(d);
            return true;
        } else if (rc.canMove(d) && rc.senseRubble(rc.getLocation().add(d.rotateRight())) < 50) {
            d = d.rotateRight();
            rc.move(d.rotateRight());
            return true;
        } else if (rc.canMove(d) && rc.senseRubble(rc.getLocation().add(d.rotateLeft())) < 50) {
            d = d.rotateLeft();
